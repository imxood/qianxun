...
impl MemoryManager {
    ...
    pub async fn write_memory(&self, summary: &str, tags: &[&str], body: &str) {
        ...
        self.write(&format!("{timestamp}.md"), &content).await;
    }
    ...
    pub async fn build_context(&self) -> String {
        ...
        for path in files.iter().take(self.recent_count) {
            if let Some((meta, body)) = Self::parse_memory_file(path).await {
    ...
    async fn list_memory_files(&self) -> Vec<PathBuf> {
        let dir = self.memory_dir();
        let mut files: Vec<PathBuf> = match fs::read_dir(&dir).await {
            Ok(mut rd) => {
                let mut out = Vec::new();
                loop {
    ...
    /// 解析记忆文件，返回 (元数据, 正文)。
    async fn parse_memory_file(path: &Path) -> Option<(HashMap<String, String>, String)> {
        let content = fs::read_to_string(path).await.ok()?;
...
mod tests {
    ...
    async fn test_memory_read_write() {
        ...
        mm.write_memory("测试摘要", &["test"], "这是正文内容").await;

        let files = mm.list_memory_files().await;
        assert_eq!(files.len(), 1, "应该有一个记忆文件");

        // 通过 read 验证正文
        let name = files[0].file_name().unwrap().to_str().unwrap();
        let content = mm.read(name).await;
    ...
    async fn test_memory_build_context() {
        ...
        for i in 0..5 {
            mm.write_memory(&format!("记忆 {i}"), &["test"], &format!("正文内容 {i}")).await;
    ...
    async fn test_memory_persistence() {
        ...
        // 写入一条记忆
        {
            ...
            mm.write_memory("持久化测试", &["persist"], "应在之后可读").await;
        } // MemoryManager 被丢弃
        ...
        // 用相同目录重建
        {
            ...
            let content = mm.read(files[0].file_name().unwrap().to_str().unwrap()).await;
    ...
    async fn test_memory_empty() {
        ...
        assert!(mm.build_context().await.is_empty(), "空记忆目录应返回空字符串");
        let files = mm.list_memory_files().await;
        assert!(files.is_empty(), "空目录应无记忆文件");
