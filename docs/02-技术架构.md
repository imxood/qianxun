...
设置持久化在数据根的 `settings.json`，带 `schemaVersion`，升级时做迁移。
Rust 侧为唯一读写者（原子写：临时文件 + rename），前端通过 IPC 读写。

数据根按构建类型拆分（详见 `src-tauri/src/paths.rs`）：

- 安装版（release）：`%USERPROFILE%\.qianxun`
- 调试版（`pnpm dev`）：`%USERPROFILE%\.qianxun_dev`

debug 后缀隔离避免开发期间改设置、重装 DSH、刷硬编码版本时污染生产数据；
第一次启用新位置的 dev 构建会按完整流程重新装一份 DSH、设置回到默认。
旧版基于 `app_data_dir()` 的位置不再写入。
...
```jsonc
{
  ...
  "dsh": {
    ...
    "versionStrategy": "pinned", // DSH 版本由千寻硬编码（install::PINNED_VERSION），随千寻版本发布，用户不可改
    "autostart": true, // 随千寻启动自动拉起 DSH
    "home": "isolated", // isolated=应用数据目录下独立 DSH_HOME（ADR-009） | system=~/.dsh
  },
  "mirrors": {
    "nodeBinary": "auto", // auto=npmmirror 优先失败落官方 | official | npmmirror
...
### 4.3 多标签终端（v0.1 交付，v0.5 移除）

终端域已在 v0.5 整体移除（Rust `terminal/`、前端 `features/terminal/`、
portable-pty 与 xterm 三件套依赖、settings.terminal 偏好、PIN 记录 IPC 全删）——
最新 DSH 已内置同等能力的终端，外壳不再重复承载。历史设计见 git 与
`docs/05-功能深化设计-v0.2.md`；用户数据目录 `~/.qianxun/terminals/`
（PIN 回放记录）保留在盘，不做静默清理。

### 4.4 环境检测与安装（v0.1 已交付）

- **Node**：`node-runtime` crate 全盘探测（含 nvm/fnm/Volta 等不在 PATH 的安装），
  满足最低版本（22.19+）者取最新；未检测到 → 检测行即安装按钮；
  下载源按 `mirrors.nodeBinary`：`auto`（npmmirror 优先，失败落官方）/ 强制官方 / 强制 npmmirror，
  安装前核对 SHA-256，落在应用数据目录，不动 PATH；
- **DSH**：未检测到 → 检测行即安装按钮；**安装走 pnpm**（每次安装经 npm 把
  `pnpm@latest` 刷进 `pnpm-tool/`，刷新失败回落已装版本）：DSH 依赖树带 peer 链（如 cordis-plugin-group），
...
- **DSH 0.1.2+ 浏览器鉴权由网关代持**：DSH 就绪 URL 里的启动 token
  只用于 `GET /?token=…` 兑换 Host 绑定的签名 cookie（`dsh-auth-*`），
  `/api/*` 与 WS 只认 cookie。网关在启动时（及上游 401 时）自动完成
  兑换并把 cookie 附在每次上游请求上；token 与 DSH cookie 都不出网关
  进程，手机侧只持有千寻自己的 `qx_token`。转发时剥除手机的
  Host/Origin/Sec-Fetch 头（DSH 的 Host/Origin 栅栏按上游 authority 判定）；
...
插件 = 装进 DSH profile 的 npm 包：pnpm 精确版本装入 + 包名并进 profile
manifest 的 `dsh.profile.bundles`。**浏览在前端，变更在 Rust**：

- **前端（浏览/发现）**：webview 直接 fetch——npmmirror 的 search /
  packument / tarball 端点全带 CORS（实测），且走系统代理（WebView2 网络
  栈），比 Rust reqwest 更适应国内代理环境；CSP connect-src 放行 `https:`。
  双数据源（`lib/market/`）：
  - **推荐**：`dsh-plugin-catalog`（awesome-dsh-plugin 社区策展的每日构建，
    npm 包承载、镜像内直达）——tar.gz 由前端解包（DecompressionStream +
    手写 tar reader，零依赖）；含分类（自带中文标签）、双语描述、星数/
    下载量；一期只展示发布了 npm 包的条目；10 分钟内存 TTL，失败即报错
    不静默降级；
  - **搜索**：registry `/-/v1/search`（默认 npmmirror，空词给 `dsh`），
    去中心化兜底；
  - 详情：`{registry}/{name}/latest` manifest，兼容结论用前端宽松 semver
    （`lib/market/semver.ts`，与 Rust 判定同规则）。
- **Rust（变更面，唯一信任边界）**：`market/mod.rs` 三个命令
  （`market_installed` / `market_install` / `market_remove`）。安装前
  `market/registry.rs` 用 registry 详情复核（不信任前端结论）：版本与
  registry 最新一致、弃用拒绝、兼容冲突拒绝（rc 钉版宽松判定，只拦确凿
  不兼容）；千寻自带 pnpm 直跑 profile 目录（超时/进程树回收复用
  `install::run_with_limits`），bundles 数组 serde_json 原子改写，首次
  幂等写入 profile 的 pnpm 构建白名单；`@deepseek-ai/*` 内核不可卸载。
- 变更即时落盘，DSH 下次启动生效；pnpm 输出行进 supervisor 日志，
  插件页底部跟一行最新日志当进度；
- UI（插件页）：推荐 / 搜索 / 已安装三个 tab——推荐含分类 chips 与本地
  过滤（分页渲染），排序默认下载数降序（可切最近收录 / 星数 / 目录原序），
  搜索默认相关性（可切更新时间 / 下载数 / 名称）；已安装来自 profile
  manifest（dependencies × bundles × node_modules 落盘）三态标注
  （生效中 / 未加载 / 内核）。千寻笔记桥
  （qx-bridge）是千寻自有插件，不走市场，部署入口在笔记页状态条。
...
| 015 | **千寻版本号语义**：主版本号第二位 = DSH 适配锚点，第三位 = 千寻修订号 | 版本号一眼可读「这次发版是否动了 DSH」；锚点切换 = 一次 DSH 升级 = 一次完整真机验收；修订号自增只做千寻自身迭代不重验 DSH | 锚点代号是约定不是机械推导（`0.4` ↔ `0.1.5` 由团队手工映射）；每次升锚点必须更新 `install::PINNED_VERSION` 并跑 DSH 全套验收 |
