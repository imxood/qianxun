...
manifest 的 `dsh.profile.bundles`。目录不精选——搜索直接透传用户配置的
registry（`mirrors.npmRegistry`，默认 npmmirror）的 `/-/v1/search`；空搜索词
给生态发现词 `dsh`。

- `market/registry.rs`（只读网络）：搜索（Listing：名称/版本/描述/发布者/
  周下载/更新时间）与详情（`{base}/{name}/latest` manifest：`dsh.bundle.patch`
  有无区分插件与普通包、`@deepseek-ai/dsh` 依赖范围 vs `PINNED_VERSION` 的
  兼容结论（rc 钉版按宽松判定）、生命周期脚本、弃用标记、包体积）；
- 安装/卸载（`market/mod.rs`）：千寻自带的 pnpm 直跑 profile 目录
  （`--dir <profile> add name@精确版本`），超时/进程树回收复用
  `install::run_with_limits`；bundles 数组经 serde_json 原子改写（其余字段
  原样保留）；首次安装幂等写入 profile 的 pnpm 构建白名单；
- 硬门槛：弃用包、兼容性冲突拒绝安装（安装前用详情复核 registry 最新版本
  与请求版本一致）；`@deepseek-ai/*` 内核组件不可卸载；
- 变更即时落盘，DSH 下次启动生效；pnpm 输出行进 supervisor 日志，
  插件页底部跟一行最新日志当进度；
- UI（插件页）：发现 / 已安装两个 tab；行内展开详情；已安装来自 profile
  manifest（dependencies × bundles × node_modules 落盘）三态标注
  （生效中 / 未加载 / 内核）。千寻笔记桥（qx-bridge）是千寻自有插件，
  不走市场，部署入口在笔记页状态条。
