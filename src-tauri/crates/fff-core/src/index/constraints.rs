...
enum GlobStrategy {
    ...
    /// Batch all paths through globset once; per-item check is a Vec<bool> lookup.
...
fn evaluate<T: Constrainable>(
    ...
    let raw = match constraint {
        ...
        Constraint::Parts(_)
        | Constraint::Exclude(_)
        | Constraint::FileType(_)
        | Constraint::GitStatus(_) => true,
    };
...
/// (no pre-filter) takes prepass — single batched glob matcher call beats N inline matches.
...
fn match_glob_pattern(pattern: &str, paths: &[&str]) -> Vec<bool> {
    ...
    let Some(p) = compile_one(pattern) else {
        return mask;
    };
    if paths.len() >= PAR_THRESHOLD {
        ...
        mask.par_iter_mut()
            .zip(paths)
            .for_each(|(slot, path)| *slot = p.is_match(path));
    } else {
        for (slot, path) in mask.iter_mut().zip(paths) {
            *slot = p.is_match(path);
        }
    }
    mask
}
