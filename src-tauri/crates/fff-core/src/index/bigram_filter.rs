...
fn read_bigram_chunk<'a>(
    file: &FileItem,
    #[allow(unused_variables)] base_fd: (),
    ...
    #[cfg(not(unix))]
    let filled = file.read_trimmed_into_buf(-1, base_path, arena, path_buf, &mut buf[..want]);
    #[cfg(unix)]
    let filled = file.read_trimmed_into_buf(0, base_path, arena, path_buf, &mut buf[..want]);
    if filled == 0 {
        return None;
    }
