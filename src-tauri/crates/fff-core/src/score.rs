...
fn match_fuzzy_parts(
    ...
    if valid_parts.is_empty() {
        log::debug!("match_fuzzy_parts: no valid parts after filtering, returning empty");
        return vec![];
    }
...
fn score_dirs_by_frecency<'a>(
    ...
    // No frecency scoring without frecency - score all dirs equally
...
fn match_and_score_in_arena<'a>(
    ...
    let results: Vec<_> = path_matches
        ...
        .map(|(match_idx, path_match)| {
            ...
            let base_score = path_match.score as i32;
            let _frecency_boost = 0;

            let _git_status_boost = 0;
            ...
            // Combo match boost removed with frecency
            let combo_match_boost = 0i32;
...
fn score_filtered_by_frecency<'a>(
    ...
    let score_file = |file: &'a FileItem| {
        let current_file_penalty = calculate_current_file_penalty(file, 0, context, arena);
        let total = current_file_penalty;
    ...
    fn make_test_files(specs: &[(&str, i32, u64)]) -> (Vec<(FileItem, Score)>, ArenaPtr) {
        ...
        let items: Vec<FileItem> = specs
            ...
            .map(|(p, _, _)| {
                ...
                FileItem::new_raw(fname, 0, 0, false)
            })
            .collect();
        let (store, strings) =
        ...
        let result: Vec<(FileItem, Score)> = specs
            ...
            .map(|(i, &(_, score, modified))| {
                let filename_start = path_strings[i]
                    ...
                    .map(|j| j + 1)
                    .unwrap_or(0) as u16;
                let mut file = FileItem::new_raw(filename_start, 0, modified, false);
    ...
    fn test_full_path_query_prefers_closer_filename_match() {
        ...
        assert_eq!(
            ...
            "near-exact full-path match should rank first, but got: {} \
             (total={}, base={})",
            results[0].0, results[0].1.total, results[0].1.base_score,
        );
    }
