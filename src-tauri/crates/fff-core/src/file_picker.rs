...
//! # Rescan Reasons
//!
//! `SharedFilePicker` exposes `trigger_full_rescan_with_reason()` so the
//! watcher can signal *why* a full rescan is needed, allowing telemetry or
...
use fff_query_parser::{FFFMode, FFFQuery};
...
/// Reasons why a full filesystem rescan was triggered by the watcher.
#[derive(Debug, Clone, Copy)]
pub enum RescanReason {
    /// Kernel dropped inotify/FS events (Linux only).
    KernelEventLoss,
    /// An ignore/config file changed on disk.
    IgnoreFileChanged,
    /// Too many debounced events accumulated; fall back to full scan.
    EventBatchOverflow,
    /// The in-memory index rejected an incremental update (capacity / lock).
    IndexUpdateRejected,
    /// The in-memory index hit capacity limits and must be rebuilt.
    OverflowCapacity,
}
...
impl FileItem {
    pub fn new(path: PathBuf, base_path: &Path) -> (Self, String) {
        let metadata = std::fs::metadata(&path).ok();
        Self::new_with_metadata(path, base_path, metadata.as_ref())
    }
    ...
    fn new_with_metadata(
        ...
        let item = Self::new_raw(filename_start, size, modified, is_binary);
        (item, relative_path)
    }
    ...
    pub fn new_from_walk(
        ...
        Self::new_from_walk_parts(path, base_path, size, modified)
    }
    ...
    pub fn new_from_walk_parts(
        ...
        let item = Self::new_raw(fname_offset, size, modified, is_binary);
        (item, rel_str)
    }
    ...
    pub fn new_from_walk_bytes(
        ...
        let item = Self::new_raw(basename_offset, size, modified, is_binary);
        (item, decoded)
    }
}
...
impl FilePicker {
    ...
    pub fn new(options: FilePickerOptions) -> Result<Self, Error> {
        ...
        let trace_id = crate::constants::TRACE_ID_LEN;
        ...
        Ok(FilePicker {
            ...
            trace_id: String::with_capacity(trace_id),
        })
    }

    /// Create a picker, place it into the shared handle, and spawn background
    /// indexing + file-system watching — the default entry point.
    ...
    pub fn collect_files(&mut self) -> Result<(), Error> {
        ...
        if !self.has_explicit_cache_budget {
            ...
            self.cache_budget = Arc::new(ContentCacheBudget::unlimited());
        }
    ...
    /// Re-index the entire filesystem synchronously.
    pub(crate) fn rescan_sync(&mut self) -> Result<(), Error> {
        self.signals.scanning.store(true, Ordering::Relaxed);
        self.scanned_files_count.store(0, Ordering::Relaxed);
    ...
    pub fn fuzzy_search<'q>(
        ...
        debug!(
            "Fuzzy search: query='{}', pagination={:?}, max_threads={}, current_file={:?}",
            query.raw_query, options.pagination, max_threads, options.current_file
        );
        ...
        info!(
            "Fuzzy search completed: query='{:?}', completed_in={:?}, total_matched={}, returned_count={}, pagination={:?}",
            query.raw_query,
            time.elapsed(),
            total_matched,
            items.len(),
    ...
    pub fn fuzzy_search_directories<'q>(
        ...
        info!(
            "Directory search completed: query='{}', completed_in={:?}, total_matched={}, returned_count={}",
            query.raw_query,
            time.elapsed(),
            total_matched,
            items.len()
    ...
    pub fn fuzzy_search_mixed<'q>(
        ...
        let file_results = self.fuzzy_search(query, file_options);
    ...
    pub(crate) unsafe fn post_scan_snapshot(&self) -> Option<PostScanUnsafeSnapshot> {
        if self
            ...
            .is_err()
        {
            log::error!("Can not acquire post scan unsafe snapshot, someone already acquired it");
            return None;
        }
    ...
    pub fn add_new_file(&mut self, path: &Path) -> Option<&FileItem> {
        ...
        let canonical_buf: Option<PathBuf> = if path.starts_with(&self.base_path) {
            ...
            log::error!(
                "Failed to canonicalize file path to add: {}",
                path.display()
            );
            return None;
        };
        ...
        let (mut file_item, rel_path) =
            FileItem::new(path_for_index.to_path_buf(), &self.base_path);
...
impl FileSync {
    /// Returns files immediately (searchable).
    #[allow(dead_code)]
    pub(crate) fn walk_filesystem(
        ...
        info!("SCAN: Starting filesystem walk");
        ...
        // un-indexable files that are binary or not fitting the size cap has to be placed in the end
...
/// Warmup mmaps is a no-op on Windows (no mmap, uses std::fs::read).
#[allow(dead_code)]
pub(crate) fn warmup_mmaps(
    ...
    // No-op: Windows uses std::fs::read instead of mmap
}
...
mod tests {
    ...
    fn dir_table_merges_walked_dirs_with_file_parents() {
        let mut pairs: Vec<(FileItem, String)> = ["src/main.rs", "src/deep/lib.rs", "root.txt"]
            .iter()
            .map(|p| {
                let (item, rel) = FileItem::new(PathBuf::from(p), Path::new(""));
                (item, rel)
            })
            .collect();
    ...
    fn directory_removal_collects_each_tombstoned_path() {
        ...
        // 索引内路径是 '/'-canonical（可能带 verbatim `\\?\` 前缀，分隔符
        // 混用）。比较前剥掉前缀并统一分隔符，不依赖平台形态。
        let normalize = |p: &std::path::Path| {
            p.to_string_lossy()
                .trim_start_matches(r"\\?\")
                .replace('\\', "/")
        };
        let mut expected = vec![normalize(&first), normalize(&second)];
        expected.sort_unstable();
        let mut removed: Vec<String> = removed.iter().map(|p| normalize(p)).collect();
