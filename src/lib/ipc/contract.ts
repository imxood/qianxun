...
/** 目录/文件条目。`children` 永远为空——drill 必新发起一次 `disk_scan_stream`，
 * 千寻 UI 不渲染深层缩进。下钻路径一律走流式扫描，避免把整棵子树塞到 IPC。 */
export interface DiskEntry {
  /** 显示名；占位项为「其余 N 项」（占位项 path 为空串、dir=false）。 */
  name: string;
  /** 绝对路径；占位项为空串（不可下钻/清理）。 */
  path: string;
  /** 递归占用（目录）/ 文件大小（文件）。遍历中此值为部分累加。 */
  size: number;
  /** 是否目录（方块可下钻）。 */
  dir: boolean;
  /** 保留字段，UI 端始终视为空。 */
