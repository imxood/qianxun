# 配置 Qianxun Web Search

[English](configure.md) | 简体中文

用户问怎么装 qx-websearch、要加 key、要换引擎，或者遇到配置类的失败时，读这份文档。直接替用户把命令跑了，不要把说明贴给对方。
...
qx-websearch 没有配置文件也能跑：抓取有 `local` 兜底，搜索走 Firecrawl 的免注册免费额度。搜索没有 local 后备（`local` 不能搜索），所以 Firecrawl 被限流时需要一个带 key 的引擎或 `agy`。它看这台机器上有什么，用最好的那个。只有用户想改变这一点时才需要建配置。
...
`~/.qianxun/websearch/config.json`，写入时权限 0600，展示时 key 打码。优先级：CLI 参数 > 环境变量 > 这个文件 > 内置默认值。

```bash
qx-websearch config init     # 生成起步文件，每个字段都可省略
qx-websearch config show     # 生效配置：文件与环境变量合并，每个值标注来源 (file)/(env)，key 打码，别名归一显示
```
...
```bash
qx-websearch config set engine tavily            # 选定搜索引擎
qx-websearch config set engine ""                # 回到自动
qx-websearch config set tavily.apiKey <key>      # 引擎凭据
qx-websearch config set tavily.apiKey <key1,key2> # 按此顺序轮换 key
qx-websearch config set tavily.apiKey            # 不带值：隐藏输入提示（见下）
qx-websearch config set tavily.baseURL <url>     # 兼容的第三方端点
qx-websearch config set tavily.enabled false     # 不让 Tavily 参与自动故障转移
qx-websearch config set tavily.enabled true      # 删除禁用覆盖
qx-websearch config set cooldown off             # 关闭额度冷却故障转移
qx-websearch config set allowPrivateNetwork true # 允许访问保留/私有地址段
```

用户要往对话里贴 key 时，先给更干净的路径：让用户在自己的终端跑不带值的
`qx-websearch config set <engine>.apiKey`，CLI 会弹出不回显的输入提示，key 不进这
个对话、不进 argv、不进 shell 历史（`pbpaste | qx-websearch config set
...
```bash
qx-websearch config set antigravity-cli.model gemini-3.1-pro-high   # 更难的研究型问题
...
qx-websearch config set firecrawl.keylessFetch false   # 让自动抓取不走云端
```
...
每次 Firecrawl 抓取花一个 credit 并强制重新爬。qx-websearch 发送 `maxAge: 0`，关掉 Firecrawl 默认的多天缓存，所以抓取永远不会拿到过期内容。这个取舍是故意的：一次抓取一个 credit，换来内容是新的，这正是工具的意义。想省 credits 不要新鲜度的话，Firecrawl 不是该选的引擎。
...
qx-websearch config set tavily.baseURL ""        # 回到官方端点
...
qx-websearch config set allowPrivateNetwork true   # 永久生效（顶层，全局）
```

## 额度冷却故障转移

某个 API key 因额度类错误失败时，qx-websearch 把这个 key 记在 `~/.qianxun/websearch/state.json`（与 `config.json` 分开）。下一次运行会先试同一引擎里的健康 key。只有所有已配置 key 都在冷却时，整个引擎才会移到后备链末尾。没有 API key 的引擎仍按引擎粒度冷却。这是软化的熔断器，不是负载均衡：健康 key 和健康引擎拿全部工作，冷却项仍保留为最后尝试。
...
```bash
qx-websearch config set cooldown off   # 关闭：不读不写状态，路由与从前完全一致
qx-websearch config set cooldown on    # 重新打开
qx-websearch state clear               # 立即忘掉所有冷却
```

`qx-websearch doctor` 会显示开关状态和当前在冷却的每个引擎或 key，以及剩余时间。
...
- 配置里引擎名写错：qx-websearch 在 `warnings` 里说明，并照常用一个能干活的引擎。用户想要回那个引擎时把名字改对。
