...
With the skill installed you do not type commands: ask anything that needs checking, or paste a URL, and it fires on its own, with the launcher choosing how to run qx-websearch. The commands below are for driving the CLI yourself on a machine with Node:

```bash
qx-websearch -q "current Node.js LTS version"     # search the web
qx-websearch -u "https://nodejs.org/en/about"     # read one page, add -q for a focus
qx-websearch -q "reactions on X" --source x       # search X, automatic for queries about X
```
...
Configuration is optional. `~/.qianxun/websearch/config.json` can name a preferred engine (`qx-websearch config set engine tavily`, empty means automatic) and exclude individual engines from automatic failover (`qx-websearch config set tavily.enabled false`). Every engine participates when no override exists. Quota cooldown failover is on by default, `qx-websearch config set cooldown off` disables it, and `qx-websearch state clear` resets the cooldown records. The full file structure and every field, including the top-level `allowPrivateNetwork` switch, are documented in the [configuration doc](configure.md).

`qx-websearch doctor` prints a local diagnosis: Node version, each task's engines with readiness and enabled state kept separate, where each config value comes from, the private-network setting, and any engines currently cooling. It spends no quota and makes no network request, and `--json` makes the output machine-readable. Run it first when routing does not behave as expected.

## Platform support

macOS and Linux are fully supported and run the whole test suite in CI on Node 22 and 24. The skill ships two launchers, `scripts/run.sh` for macOS and Linux and `scripts/run.ps1` for Windows, which pick a working way to run qx-websearch automatically and behave the same on all three platforms.
...
- **agy and grok are external CLIs.** qx-websearch runs them by name with no shell, so a native Windows executable on PATH works, while an npm-style `.cmd` shim does not. Whether a Windows build exists is each tool's own decision, not qx-websearch's. Every direct child is started with `windowsHide: true`, so a GUI host with no console does not flash a new black window per call.
