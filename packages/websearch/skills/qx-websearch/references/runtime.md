# qx-websearch runtime reference

English | [简体中文](runtime.zh-CN.md)

How the skill launches the `qx-websearch` CLI, what version it pins, and how it
...
- npm package: `qx-websearch`
- CLI binary name: `qx-websearch`
...
1. **A compatible `qx-websearch` already on `PATH`** — run it directly, by name.
2. **`npx` present, and `node` meets the CLI's 22.13 floor** — `npx --yes --package qx-websearch@<pinned> qx-websearch <args>`. An npx sitting on an older node is skipped: it would select a path known to fail at run time.
3. **`bunx` present** — `bunx --bun qx-websearch@<pinned> <args>`.
...
A `qx-websearch` found on `PATH` counts as compatible only when it is **the same
...
- `os`, `arch` — normalized host identity (`darwin` / `linux` / `windows`,
  `arm64` / `x64`).
- `checked.pathCli` — `{ present, path, version, compatible }` for a `qx-websearch`
...
qx-websearch stays a local CLI on purpose, and section 10 of the distribution
...
retire native artifacts for some future tool, but qx-websearch keeps its local-CLI
