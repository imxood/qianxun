...
装好 skill 后不需要敲命令：问任何需要查证的问题，或贴一个 URL，它就会自己触发，启动器负责选择怎么运行 qx-websearch。下面的命令用于在有 Node 的机器上自己驱动 CLI：

```bash
qx-websearch -q "current Node.js LTS version"     # 搜网页
qx-websearch -u "https://nodejs.org/en/about"     # 读一个页面，加 -q 指定关注点
qx-websearch -q "reactions on X" --source x       # 搜 X，关于 X 的查询会自动路由过去
```
...
配置是可选的。`~/.qianxun/websearch/config.json` 可以指定首选引擎（`qx-websearch config set engine tavily`，空表示自动），也可以把单个引擎排除在自动故障转移之外（`qx-websearch config set tavily.enabled false`）。没有覆盖时每个引擎都参与。额度冷却故障转移默认开，`qx-websearch config set cooldown off` 关掉，`qx-websearch state clear` 重置冷却记录。完整的文件结构和每个字段，包括顶层的 `allowPrivateNetwork` 开关，见[配置文档](configure.zh-CN.md)。

`qx-websearch doctor` 打印本机诊断：Node 版本、每项任务的引擎就绪状态与启用状态、每个配置值的来源、私有网络设置、当前在冷却的引擎。就绪与启用分开报告。它不花额度、不发网络请求，`--json` 让输出可被程序读取。路由行为不符合预期时先跑它。
...
- **agy 和 grok 是外部 CLI。** qx-websearch 不经 shell 按名字直接运行它们，所以 PATH 上的原生 Windows 可执行文件可用，npm 风格的 `.cmd` 垫片不可用。有没有 Windows 构建是每个工具自己的决定，不是 qx-websearch 的。每个直接子进程都会带 `windowsHide: true` 启动，因此没有控制台的 GUI 宿主不会在每次调用时闪出黑框。
