# Configuring Qianxun Web Search

English | [简体中文](configure.zh-CN.md)

Read this when the user asks how to set up qx-websearch, wants a key added, wants a different engine, or hits a setup-related failure. Run the commands for them instead of pasting instructions.
...
`~/.qianxun/websearch/config.json`, written 0600, keys masked when shown. Precedence: CLI flags > environment variables > this file > built-in defaults.

```bash
qx-websearch config init     # starter file, every field optional
qx-websearch config show     # effective config: file + env merged, each value tagged (file)/(env), keys masked, alias keys shown canonical
```
...
```bash
qx-websearch config set engine tavily            # choose the search engine
qx-websearch config set engine ""                # back to automatic
qx-websearch config set tavily.apiKey <key>      # engine credentials
qx-websearch config set tavily.apiKey <key1,key2> # rotate keys in this order
qx-websearch config set tavily.apiKey            # no value: hidden prompt (see below)
qx-websearch config set tavily.baseURL <url>     # a compatible third-party endpoint
qx-websearch config set tavily.enabled false     # keep Tavily out of automatic failover
qx-websearch config set tavily.enabled true      # remove the opt-out
qx-websearch config set cooldown off             # turn off quota cooldown failover
qx-websearch config set allowPrivateNetwork true # reach reserved/private ranges
```

When the user is about to paste a key into the chat, offer the cleaner path
first: run `qx-websearch config set <engine>.apiKey` with no value in their
terminal, and the CLI prompts with the echo muted, so the key never enters
this conversation, argv, or their shell history (`pbpaste | qx-websearch config
...
```bash
qx-websearch config set antigravity-cli.model gemini-3.1-pro-high   # harder research questions
qx-websearch config set antigravity-cli.bin /custom/path/to/agy
```
...
```bash
qx-websearch config set tavily.apiKey <key>
...
```bash
qx-websearch config set exa.apiKey <key>
...
```bash
qx-websearch config set firecrawl.apiKey <key>
# or environment: export FIRECRAWL_API_KEY=<key>
qx-websearch config set firecrawl.keylessFetch false   # keep automatic page fetch off the cloud
```
...
```bash
qx-websearch config set tavily.baseURL https://gw.example.com/tavily
qx-websearch config set tavily.baseURL ""        # back to the official endpoint
...
```bash
qx-websearch config set cooldown off   # disable: read and write no state, route exactly as before
qx-websearch config set cooldown on    # re-enable
qx-websearch state clear               # forget every cooldown now
```

`qx-websearch doctor` shows the switch and every engine or key cooling right now, with the time left.
...
- Wrong engine name in config: qx-websearch says so in `warnings` and uses a working engine anyway. Fix the name when the user wants that engine back.
