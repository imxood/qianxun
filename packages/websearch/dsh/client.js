// Browser half of the qx-websearch dsh plugin: the settings card.
//
// dsh web users have no terminal, so `qx-websearch config set` is out of reach
...
// ~/.qianxun/websearch/config.json: the browser never sees an API key, and never
  ...
  factory: (require) => {
    ...
    var TEXT = {
      en: {
        ...
        apiKey: 'API key',
        apiKeyHint:
          'Separate multiple keys with commas. Qianxun Web Search rotates to the next key after authentication, rate-limit, or quota failures.',
        baseUrl: 'Base URL',
        model: 'Model',
    ...
    function ConfigCard(react, ui, localeRef) {
      ...
      return function QxWebsearchCard() {
        ...
        var load = react.useCallback(() => {
          ...
          fetch('/qx/websearch/config?doctor=1')
            .then((r) =>
              r.json().then((body) => {
                if (!r.ok) {
        ...
        fetch('/qx/websearch/config')
          .then((response) => {
            ...
            try {
              ...
              console.error(`[qx-websearch] settings card skipped: ${error}`);
            }
          })
          .catch(() => {});
      });
    ...
    function mountCard(ctx, localeRef) {
      ...
      ctx.slots.inject('settings.plugin.item', function* () {
        ...
        yield ctx.slots.register(
          { name: 'settings.plugin.item', id: 'qx-websearch', key: 'qx-websearch', order: 31 },
          Card,
        );
      });
    }
