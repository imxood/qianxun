...
// Layered configuration: CLI flags > environment variables > ~/.qianxun/websearch/config.json > built-ins.
//
...
export const CONFIG_DIR = path.join(os.homedir(), '.qianxun', 'websearch');
export const CONFIG_PATH = path.join(CONFIG_DIR, 'config.json');

/** Resolved at call time so a faked HOME (tests) redirects the config too. */
export function currentConfigPath(): string {
  return path.join(os.homedir(), '.qianxun', 'websearch', 'config.json');
}
 ...
 */
const SEEN_API_KEYS = new WeakMap<WebsearchConfig, string[]>();

export function migrateLegacyConfig(raw: WebsearchConfig & LegacyConfig): WebsearchConfig {
...
export function loadConfigFile(configPath = currentConfigPath()): WebsearchConfig {
  let raw: string;
  try {
  ...
  try {
    ...
    return migrateLegacyConfig(parsed as WebsearchConfig & LegacyConfig);
  } catch (error) {
    throw new Error(
...
function writeConfigFile(config: WebsearchConfig, configPath: string): void {
 ...
 */
export const CONFIG_TEMPLATE: WebsearchConfig = {
...
function knownApiKeys(config: WebsearchConfig, env: NodeJS.ProcessEnv): string[] {
...
/**
 * Render the effective config: what qx-websearch will actually use, not just what
