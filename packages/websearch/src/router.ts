import { chosenEngine, engineSettings, type WebsearchConfig, type Role } from './config.ts';
...
export function planRole(
  ...
  if (explicit) {
    ...
    if (!engine) {
      notes.push(
        `Unknown engine "${explicit}" (--engine). Drop -e to let qx-websearch pick one that works, or name a known engine: ${listEngines().join(', ')}.`,
      );
    } else if (!engine.roles.includes(role)) {
      notes.push(
        `The ${engine.name} engine cannot ${role} (--engine forces it with no fallback). Drop -e to let qx-websearch pick an engine that can. ${engine.name} handles: ${engine.roles.join(', ')}.`,
      );
    } else {
      add(engine);
    }
  ...
  if (configured) {
    ...
    if (!engine) {
      notes.push(
        `Unknown engine "${configured}" (engine in the config file), so qx-websearch chose one that works.`,
      );
    } else if (engine.roles.includes(role)) {
      if (enabled(engine)) {
        add(engine);
        ...
        notes.push(
          `The configured ${engine.name} preference is disabled by engines.${engine.name}.enabled, so qx-websearch skipped it.`,
        );
      }
    }
