...
describe.runIf(SPAWNS_FAKE_CLI)('runCommand timeout handling', () => {
  it('SIGKILLs a child that ignores SIGTERM, so its PID goes away', async () => {
    const dir = tempDir('qx-websearch-sigkill-');
...
describe.runIf(SPAWNS_FAKE_CLI)('runCommand child environment', () => {
  it('sets QX_WEBSEARCH_NESTED=1 on the child and keeps inherited env', async () => {
    const dir = tempDir('qx-websearch-nested-env-');
    const bin = path.join(dir, 'echo-env');
    fs.writeFileSync(
      bin,
      '#!/bin/sh\nprintf "NESTED=%s\\nSENTINEL=%s\\n" "$QX_WEBSEARCH_NESTED" "$QX_WEBSEARCH_TEST_SENTINEL"\n',
      { mode: 0o755 },
    );
    const previous = process.env.QX_WEBSEARCH_TEST_SENTINEL;
    process.env.QX_WEBSEARCH_TEST_SENTINEL = 'keep-me';
    try {
      ...
      if (previous === undefined) {
        delete process.env.QX_WEBSEARCH_TEST_SENTINEL;
      } else {
        process.env.QX_WEBSEARCH_TEST_SENTINEL = previous;
      }
    }
  });
});
