...
const DOC_URL = new URL('../skills/qx-websearch/references/output-schema.md', import.meta.url);
...
describe('output-schema reference stays in step with RunSearchResult', () => {
  ...
  it('keeps the Chinese translation structurally identical to the English doc', () => {
    ...
    const zh = envelopes(
      fs.readFileSync(
        new URL('../skills/qx-websearch/references/output-schema.zh-CN.md', import.meta.url),
        'utf-8',
      ),
    );
