 ...
 *
 * qx-websearch runs engine binaries with `spawn(bin, args)` and no shell, a
...
export function tempDir(prefix = 'qx-websearch-test-'): string {
...
export function tempConfigPath(): string {
  return path.join(tempDir('qx-websearch-config-'), 'config.json');
}
...
export function envWithBinaries(...binaries: string[]): NodeJS.ProcessEnv {
  const dir = tempDir('qx-websearch-bin-');
...
export function fakeEngine(options: {
  ...
  const dir = tempDir('qx-websearch-engine-');
...
export function withTempHome(): { home: string; restore: () => void } {
  const home = tempDir('qx-websearch-home-');
