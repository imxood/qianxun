...
export async function executeExaSearch(options: EngineRequest): Promise<EngineOutput> {
  ...
  if (!apiKey) {
    throw new Error(
      'The exa provider needs an API key. Set EXA_API_KEY, or run: qx-websearch config set exa.apiKey <key> ($10/month recurring free credit, ~1,400 searches, no card at https://exa.ai)',
    );
  }
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 || response.status === 403 || response.status === 429) {
      throw new ApiKeyFailureError(
        `exa rejected the API key (${response.status}). Fix it: qx-websearch config set exa.apiKey <key>${detail ? ` (${detail})` : ''}`,
      );
    }
    throw new Error(
      `exa returned ${response.status} ${response.statusText}.${detail ? ` ${detail}` : ''}`,
