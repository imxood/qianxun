...
async function ensureOk(
  ...
  if (response.status === 401 || response.status === 403) {
    if (!apiKey) {
      throw new Error(
        `firecrawl rejected the keyless request (${response.status}). Anonymous access may be unavailable or rate-limited. Set your own key: qx-websearch config set firecrawl.apiKey <key>${detail ? ` (${detail})` : ''}`,
      );
    }
    throw new ApiKeyFailureError(
      `firecrawl rejected the API key (${response.status}). Fix it: qx-websearch config set firecrawl.apiKey <key>${detail ? ` (${detail})` : ''}`,
    );
  }
