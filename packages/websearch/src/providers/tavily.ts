...
export async function executeTavilySearch(options: EngineRequest): Promise<EngineOutput> {
  ...
  if (!apiKey) {
    throw new Error(
      'The tavily provider needs an API key. Set TAVILY_API_KEY, or run: qx-websearch config set tavily.apiKey <key> (free tier: 1,000 credits/month at https://app.tavily.com)',
    );
  }
