...
describe('the settings card mounts only where its route answers', () => {
    ...
    const definition = evaluate({
      fetch: (url: string) =>
        Promise.resolve({
          ...
          status: url.startsWith('/qx/websearch/config') ? configStatus : 200,
  ...
  it('registers under the key rc.7 dispatches by and the id rc.6 lists by', async () => {
    ...
    // serves 'qx-websearch'. rc.6's list slot needed options.id instead, and one
    ...
    expect(spec?.key).toBe('qx-websearch');
    expect(spec?.id).toBe('qx-websearch');
  });
...
describe('the card follows dsh’s own interface language', () => {
  ...
  const ZH_TITLE = '搜索引擎（千寻联网）';
  const EN_TITLE = 'Search engine (Qianxun)';
