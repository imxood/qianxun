...
describe('doctor: search engines', () => {
  ...
  it('gives a copyable fix for a missing key and missing agy', () => {
    ...
    expect(engine(report, 'search', 'tavily')?.fix).toContain('qx-websearch config set tavily.apiKey');
    expect(engine(report, 'search', 'exa')?.fix).toContain('qx-websearch config set exa.apiKey');
