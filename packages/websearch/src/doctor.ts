// `qx-websearch doctor`: diagnose configuration and routing on this machine
...
function diagnoseEngine(
  ...
  if (engine.name === 'tavily') {
    ...
    return {
      ...
      ...(ready ? {} : { fix: 'qx-websearch config set tavily.apiKey <key>' }),
    };
  }
  ...
  if (engine.name === 'exa') {
    ...
    return {
      ...
      ...(ready ? {} : { fix: 'qx-websearch config set exa.apiKey <key>' }),
    };
  }
  ...
  if (engine.name === 'firecrawl') {
    ...
    return {
      ...
      reason: keySource
        ? apiKeyPresentReason(count, keySource)
        : keylessFetch
          ? 'keyless fetch (default): public pages are read by a cloud browser, no key or signup needed. Opt out with: qx-websearch config set firecrawl.keylessFetch false'
          : 'keyless fetch is switched off (firecrawl.keylessFetch false), so Firecrawl is excluded from automatic page fetch.',
      ...(ready ? {} : { fix: 'qx-websearch config set firecrawl.keylessFetch true' }),
    };
  }
...
function candidatesForRole(role: Role, config: WebsearchConfig): SearchEngine[] {
  const names: string[] = [];
...
export function runDoctor(options: DoctorOptions = {}): DoctorReport {
  ...
  let config: WebsearchConfig = options.config ?? {};
...
export function formatDoctorReport(report: DoctorReport): string {
  const lines: string[] = ['qx-websearch doctor', ''];
