...
//
// State lives in ~/.qianxun/websearch/state.json, apart from config.json, and is a pure
...
import { canonicalEngineName, cooldownEnabled, type WebsearchConfig } from './config.ts';
...
export function currentStatePath(): string {
  return path.join(os.homedir(), '.qianxun', 'websearch', 'state.json');
}
...
/**
 * Wipe all cooldown state by deleting the file. Used by `qx-websearch state clear`.
