...
// non-.exe fake is not a runnable image under qx-websearch's no-shell spawn. They
...
describe('per-engine API key rotation', () => {
  ...
  it('records quota cooldown only for the key that failed', async () => {
    const p = path.join(tempDir('qx-websearch-key-state-'), 'state.json');
  ...
  it('tries healthy keys before a cooling key in the same engine', async () => {
    const p = path.join(tempDir('qx-websearch-key-order-'), 'state.json');
    const now = new Date('2026-08-06T00:00:00.000Z');
    recordQuotaCooldown(
      emptyCooldownState(),
      'exa',
...
describeSpawn('quota cooldown records, clears, and can be switched off', () => {
  afterEach(() => cleanupTempDirs());
  const now = new Date('2026-08-06T00:00:00.000Z');
  const statePath = () => path.join(tempDir('qx-websearch-state-'), 'state.json');
