import { engineSettings, loadConfigFile, type WebsearchConfig } from './config.ts';
...
export interface RunSearchOptions {
  ...
  /** Injected config, for tests. Loaded from ~/.qianxun/websearch/config.json otherwise. */
...
async function runOneSource(
  ...
  for (const engine of candidates) {
    ...
    // Routing facts and the warnings/attempts channels are stamped by qx-websearch,
